...
def build_clust_settings(config):
    poss_cluster = f'analyse.dbscan.(eps={config["dbscan.eps"]}, min_samples={config["dbscan.min_samples"]}, color_weight={config["dbscan.color_weight"]})'
    return poss_cluster
...
class MyWorker(Worker):
    ...
    def get_configspace():
        ...
        config_space.add_hyperparameter(CS.UniformFloatHyperparameter('dbscan.eps', lower=0.1, upper=2.5, q=0.1))
        config_space.add_hyperparameter(CS.UniformIntegerHyperparameter('dbscan.min_samples', lower=1, upper=30))
        config_space.add_hyperparameter(CS.UniformFloatHyperparameter('dbscan.color_weight', lower=0.5, upper=4, q=0.5))
        return(config_space)
...
if __name__ == '__main__':
    ...
    for p in pictures:
        NS = hpns.NameServer(run_id='dbscan1', host='127.0.0.1', port=None)
        NS.start()

        w = MyWorker(image = os.path.join(path, p), sleep_interval = 0, nameserver='127.0.0.1', run_id='dbscan1')
        w.run(background=True)

        bohb = BOHB(  configspace = w.get_configspace(),
                      run_id = 'dbscan1', nameserver='127.0.0.1',
        ...
        with open(f'res/{p[:-4]}.pkl', 'wb') as f:
            pickle.dump(res, f)
