...
def collapse_rows(rows):
    """Collapses nested lists into one list.

    Args:
        rows (list): List to collapse.
...
def build_app(distros):
    """Build the GUI, generated based on the methods in the _preprocess and _analysis scripts.

    Args:
        distros (dict): Dict with scipy.stats-distributions and labels as possibilities for fitting.

    Returns:
        sg.Window: Constructed GUI.
...
def run_app(distros):
    """Run the PySimpleGUI and check for updates.
